...
- **Active**
  ...
  - 여기에는 아래의 Reading, Writing, Waiting 상태가 모두 포함됨
...
- **Total requests**: 클라이언트가 보낸 전체 요청 횟수
  - **Connection vs Request**: 하나의 연결(Connection)을 통해 여러 개의 요청(Request)이 처리될 수 있으므로, 일반적으로 이 수치는 Processed connections보다 높게 나타냄
